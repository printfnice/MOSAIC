...
% Publication-history fields are intentionally blank for the initial submission.

\abstract{
\textbf{Motivation:} Fine-grained CITE-seq annotation under donor and protein-panel shift needs inspectable evidence.
\\
...
\\
\textbf{Availability and Implementation:} An MIT snapshot with code, configs, evidence, tests and checkpoint-free CPU validator is available at \url{https://github.com/printfnice/MOSAIC/tree/v8.1.4-submission}; raw data, caches, predictions and checkpoints are excluded.
\\
\textbf{Contact:} Linhao Wu, \url{2544814855@qq.com}.
\\
\textbf{Supplementary Information:} Supplementary tables and protocols accompany the article.
}
...
Existing annotation, reference-mapping and multimodal systems include SingleR, CellTypist, scmap, scPred, scClassify, TOSICA, scNym, scVI, scANVI, totalVI, scArches, Symphony and Azimuth \citep{aran2019,dominguezconde2022,kiselev2018scmap,alquicira2020scpred,lin2020scclassify,chen2023tosica,kimmel2021scnym,lopez2018,xu2021scvi,gayoso2021totalvi,gayoso2022scvi,lotfollahi2022,kang2021symphony,hao2021}. Curated hierarchies and programs can be strong when label space and panel match the task \citep{caron2025mmochi,kotliar2025starcat}; benchmark rankings therefore depend on modality, preprocessing and split design \citep{abdelaal2019,liu2025multitaskbenchmark,haghverdi2018,korsunsky2019,argelaguet2021}.
...
\end{aligned}
\end{equation}
The weights in $\mathcal{L}_{\mathrm{wCE}}$ are normalized $1/\sqrt{n_c}$ class weights \citep{buda2018imbalance}; stored teacher probabilities are clipped and row-normalized, while $\mathbf{p}^{T}=\operatorname{softmax}(\mathbf{z}^{\mathrm{final}}/T)$ intentionally uses the post-HSR final logits, so the KD gradient can pass through the local refinement path \citep{hinton2015distilling}. AdamW used at most 35 epochs, batch size 1,024, learning rate $10^{-3}$, weight decay $10^{-4}$, dropout 0.2 and patience 14. We fixed $\lambda_b=0.35$, $\lambda_f=0.50$, $\lambda_{\mathrm{KD}}=0.05$, $T=2$ and $\lambda_{\mathrm{HSR}}=0.20$ before testing; checkpoint selection maximized validation M-F1 subject to ACC/W-F1 within 0.0015 of their best values.
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{Fig/Fig3_MOSAIC_marker_support_audit.pdf}
\caption{\textbf{58-label marker-support audit.} Each point is one PBMC L3 label; point size is held-out test support and color indicates whether a non-empty representative ADT marker list is fully present. Panels show per-class M-F1 against ADT hits, RNA hits, total hits and mean fusion weight. The descriptive Spearman associations are near zero, so panel availability and top-20 attribution use are not interchangeable evidence types.}
\label{fig:marker-support}
\end{figure}

\subsection{Where proteins are missing matters more than how many}

At 50\% and 70\% random protein masking, W-F1 decreased to 0.8971 and 0.8921; the corresponding M-F1 losses were 0.0330 and 0.0431. RNA-only reached W-F1 0.8882, differing from the RNA branch by less than $5\times10^{-4}$, while its M-F1 was 0.7558 versus 0.7564 for the branch (Fig.~\ref{fig:evidence}e). Memory- and T-cell-marker masks gave M-F1 0.8006 and 0.7859 (Supplementary Table S8). The V8.2 size-matched null across 24 frozen donor--seed units showed W-F1 changes of $-0.000490$ and $-0.001831$ for random six- and 15-ADT masks, compared with $-0.002555$ and $-0.008693$ for the corresponding targeted masks; M-F1 changes were $-0.000158$ and $-0.004406$ versus $-0.006572$ and $-0.021330$ (Supplementary U4). Removing 15 T-cell ADTs therefore showed a larger descriptive loss than its same-count random null, rather than only reflecting feature count. Training-only 30\% ADT dropout increased the targeted-mask point estimate from 0.7859 to 0.7884 but reduced full-input M-F1 from 0.8072 to 0.8016 (the Supplementary panel-aware training sensitivity). Temperature scaling reduced ECE 0.0713 to 0.0391, NLL 0.4255 to 0.3904 and Brier 0.1523 to 0.1462 without changing argmax predictions. These are bounded panel and calibration sensitivities, not arbitrary missing-panel coverage.

\subsection{Rejection is bounded by marker availability}

At known coverage 0.80, one-minus-margin rejection gave observed coverage 0.7878, unknown recall 0.4032 and combined hierarchical accuracy 0.9146 (Supplementary Tables S9--S11). Across the five target labels and three seeds, B naive lambda was hardest (AUROC 0.4258, recall 0.1285). Its RNA light-chain genes were retained in every fold, but the representative top-20 attribution audit did not retain those genes and the ADT panel lacks a matched kappa/lambda marker (Supplementary Tables S12--S13). Parent-safe rate was 0.2259, so only about 9.1\% of unknown cells were both rejected and assigned the correct parent. This supports manual review; the 58-label audit is descriptive and does not establish a general marker--rejection law.

\subsection{Marker availability is necessary but not sufficient at CD8 memory boundaries}

On PDC101, the three-seed MOSAIC row achieved 0.9077/0.9068/0.9007 ACC/W-F1/M-F1 versus 0.9347/0.9335/0.9298 for official MMoCHi (Supplementary Tables S14--S15). In a separate exact paired audit of the valid no-HTO MOSAIC-HPM-lite artifact, 2,098 identical holdout cells gave ACC 0.9376 versus 0.9347, with 80 MOSAIC-only correct cells and 74 MMoCHi-only correct cells; the exact two-sided McNemar test was non-significant ($P=0.6871$; Supplementary U4). Gaps in the seed-aggregated comparison concentrated at CD8 memory boundaries, while monocyte F1 was tied. Representative ADT evidence is present in both tasks, so marker availability alone does not explain the PDC101 gap; hierarchy and label-space alignment remain protocol-specific alternative explanations. The MOSAIC gate is not treated as directly comparable to MMoCHi certainty because the saved quantities are different. This is a boundary-localization comparison, not a causal or superiority claim.
...
The contribution is the joint availability of modality labels/margins, traceable fusion weights, bounded six-label HSR and cell-level records. Branch supervision supplies the interface rather than the aggregate macro gain; the locked path is chosen to emit this evidence even when uniform fusion has higher aggregate metrics. Panel masks, leave-class-out targets, the 58-label marker-support audit and PDC101 expose concrete failure regimes, supporting the bounded proposition that evidence availability is a deployment resource distinct from metric optimality.

The scope remains protocol-defined: the near-capacity control does not establish strict single-module causality; CellTypist is package-compatible, DSB lacks matched controls, and the 58-label marker audit is descriptive. The size-matched null strengthens the targeted-panel interpretation but remains a fixed-panel frozen-checkpoint diagnostic, not arbitrary missing-panel robustness. The audit decision transfers to the 5P assay cohort as a same-study replication with a leakage-safe cell-level artifact, but the cohort shares the main study and P1--P8 donors and is not independent external validation; its full-accept policy also produced no fallback cells. The verified public snapshot contains source code, configurations, compact evidence and a CPU demo, but no checkpoint, immutable archive DOI or raw-data archive; the submission package additionally carries the V8.2 closure summaries and source paths. These boundaries define, rather than negate, MOSAIC's contribution: modality evidence, local action and failure localization are inspectable alongside the predicted label.

\section{Data availability}

PBMC 3P/5P are from GEO GSE164378; COVID-19 is ArrayExpress E-MTAB-10026 \citep{stephenson2021}; PDC101 is GEO GSE229791; and COMBAT is available through HCA, EGA and Zenodo DOI 10.5281/zenodo.6120249 \citep{combat2022}. Split manifests and source checks are included in the Supplementary Material.

\section{Code availability}

Code, configurations, split manifests, compact evidence, tests, environment metadata, checksums and a checkpoint-free ten-cell CPU audit validator are publicly available under MIT in the verified tagged snapshot at \url{https://github.com/printfnice/MOSAIC/tree/v8.1.4-submission}. The V8.2 closure scripts and compact summaries are included in the submission source and local release candidate; a public V8.2 tag is not asserted until remote availability is verified. Raw data, caches, full predictions and checkpoints are not redistributed; an immutable archive DOI is not assigned in the verified snapshot.
...
OpenAI Codex and the ChatGPT interface were used for code navigation, experiment-script scaffolding, artifact-audit assistance, table and metadata generation, language editing and figure-prompt iteration. They were not treated as scientific evidence and did not determine splits, held-out labels, model selection, statistical tests or conclusions. Human authors executed or reviewed the analyses, verified artifacts and approved the manuscript, figures, citations and claims; detailed tool-use documentation is provided in the Supplementary Material. Figure~1 is the author-owned final asset.
