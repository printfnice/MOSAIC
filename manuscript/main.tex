...
\\
\textbf{Availability and Implementation:} An MIT snapshot with code, configurations, evidence, tests and CPU audit demo is prepared at \url{https://github.com/printfnice/MOSAIC/tree/v8.1.2-submission}; raw data, caches, predictions and checkpoints are excluded.
\\
...
Code, configurations, split manifests, compact evidence, tests, environment metadata, checksums and a ten-cell CPU audit demo are staged under MIT in the tagged snapshot at \url{https://github.com/printfnice/MOSAIC/tree/v8.1.2-submission}. The repository visibility must be enabled for anonymous reviewer access at submission. Raw data, caches, full predictions and checkpoints are not redistributed; an immutable archive DOI is not assigned in this snapshot.
