...
\\
\textbf{Availability and Implementation:} An MIT snapshot with code, configs, evidence, tests and checkpoint-free CPU validator is prepared at \url{https://github.com/printfnice/MOSAIC/tree/v8.1.3-submission}; raw data, caches, predictions and checkpoints are excluded.
\\
...
Code, configurations, split manifests, compact evidence, tests, environment metadata, checksums and a checkpoint-free ten-cell CPU audit validator are staged under MIT in the tagged snapshot at \url{https://github.com/printfnice/MOSAIC/tree/v8.1.3-submission}. The repository visibility must be enabled for anonymous reviewer access at submission. Raw data, caches, full predictions and checkpoints are not redistributed; an immutable archive DOI is not assigned in this snapshot.
