...
\\
\textbf{Availability and Implementation:} A tagged MIT-licensed snapshot containing code, configurations, evidence, tests and a ten-cell CPU audit demo has been prepared at \url{https://github.com/printfnice/MOSAIC/tree/v8.1.1-submission}; raw data, caches, predictions and checkpoints are excluded.
\\
...
Existing annotation, reference-mapping and multimodal systems include SingleR, CellTypist, scmap, scPred, scClassify, TOSICA, scNym, scVI, scANVI, totalVI, scArches, Symphony and Azimuth \citep{aran2019,dominguezconde2022,kiselev2018scmap,alquicira2020scpred,lin2020scclassify,chen2023tosica,kimmel2021scnym,lopez2018,xu2021scvi,gayoso2021totalvi,lotfollahi2022,kang2021symphony,hao2021}. Curated hierarchies and programs can be strong when label space and panel match the task \citep{caron2025mmochi,kotliar2025starcat}; benchmark rankings therefore depend on modality, preprocessing and split design \citep{abdelaal2019,liu2025multitaskbenchmark}.
...
\end{aligned}
\end{equation}
The weights in $\mathcal{L}_{\mathrm{wCE}}$ are normalized $1/\sqrt{n_c}$ class weights \citep{buda2018imbalance}; stored teacher probabilities are clipped and row-normalized, while $\mathbf{p}^{T}=\operatorname{softmax}(\mathbf{z}^{\mathrm{final}}/T)$ intentionally uses the post-HSR final logits, so the KD gradient can pass through the local refinement path. AdamW used at most 35 epochs, batch size 1,024, learning rate $10^{-3}$, weight decay $10^{-4}$, dropout 0.2 and patience 14. We fixed $\lambda_b=0.35$, $\lambda_f=0.50$, $\lambda_{\mathrm{KD}}=0.05$, $T=2$ and $\lambda_{\mathrm{HSR}}=0.20$ before testing; checkpoint selection maximized validation M-F1 subject to ACC/W-F1 within 0.0015 of their best values.
...
Code, configurations, split manifests, compact evidence, tests, environment metadata, checksums and a ten-cell CPU audit demo are staged under MIT in the tagged snapshot at \url{https://github.com/printfnice/MOSAIC/tree/v8.1.1-submission}. The repository visibility must be enabled for anonymous reviewer access at submission. Raw data, caches, full predictions and checkpoints are not redistributed; an immutable archive DOI is not assigned in this snapshot.
