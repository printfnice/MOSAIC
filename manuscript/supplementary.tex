...
The release bundle stages code, configurations, ten transformed demonstration cells, compact evidence tables, commands, environment metadata and SHA-256 hashes. It excludes raw participant data, full caches, full predictions and checkpoints. The corresponding GitHub snapshot is tagged \texttt{v8.1.1-submission} under the MIT License and includes the CPU demo, compact evidence tables and checksum manifest; anonymous reviewer access requires the repository visibility to be enabled, while an immutable archive DOI and raw-data archive are not part of this snapshot. The panel-aware and RNA-CP10K sensitivity artifacts are included as compact evidence records rather than as raw-data redistribution.
