...
U1 & Five-target panel/rejection diagnostic (unnumbered) & Results, rejection paragraph & Representative panel availability joined to unknown recall and AUROC; v7 marker audit fragment \\
U2 & CD8 boundary linkage (unnumbered) & Results, PDC101 paragraph & PBMC attribution and PDC101 boundary comparison; v7 marker audit fragment \\
U3 & Inference-throughput diagnostic (unnumbered) & Results, computational cost & Prediction-only versus audit-record extraction throughput; v7 throughput fragment \\
U4 & V8.2 audit closure (unnumbered) & Results, panel/PDC101/breadth paragraphs & Size-matched random null, exact PDC101 paired audit and same-study 5P transfer; v8.2 closure fragment \\
...
S13 & 58-label marker/support audit & Results, marker and rejection paragraphs & Representative marker availability, attribution hits, support and fusion weights; v7 marker audit fragment \\
S14--S15 & PDC101 comparison & Results, PDC101 paragraph & External hierarchy boundary; PDC101 comparison fragments \\
S16 & Computational cost & Results, evidence map & Runtime and parameter audit; computational-cost fragment \\
S17--S18 & Strong baselines and XGBoost pairing & Results, evidence map & Comparator caveat closure; baseline and paired-comparison fragments \\
S19 & Published latent baselines & Results, evidence map & Fixed-split scANVI and totalVI context; published fixed-split baseline fragment \\
S20 & CLR preprocessing sensitivity & Results, evidence map & ADT preprocessing sensitivity; CLR sensitivity fragment \\
S21 & Attribution stability & Results, evidence map & Marker concordance and faithfulness; attribution fragment \\
S22 & Macro precision/recall decomposition & Results, evidence map & MLP comparison mechanism; macro precision/recall fragment \\
S23 & Protocol-stratified breadth matrix & Results, breadth paragraph & Secondary cross-dataset matrix; declared source matrix \\
S24 & Panel-aware training sensitivity & Supplementary panel-stress section & Training-only ADT feature dropout; panel-aware training fragment \\
S25 & RNA library-size normalization sensitivity & Results, evidence map & Full-transcriptome CP10K/log1p sensitivity; RNA normalization fragment \\
\bottomrule
\end{tabularx}
\end{center}
...
MOSAIC full versus the matched MLP produced paired differences of $+0.0016$ ACC $[-0.0011,0.0044]$ (paired $P=0.2019$), $+0.0016$ W-F1 $[-0.0009,0.0041]$ ($P=0.1681$), $+0.0111$ M-F1 $[0.0062,0.0160]$ ($P=0.0011$) and $+0.0074$ balanced accuracy $[0.0013,0.0135]$ ($P=0.0239$). Wilcoxon sensitivity $P$ values were 0.3828, 0.3828, 0.0078 and 0.0547, respectively. With eight donor units, the two-sided Wilcoxon signed-rank test has a minimum attainable $P=0.0078$; Bonferroni-adjusted confirmatory wording therefore relies on the paired $t$ test, with Wilcoxon retained as a distribution-free sensitivity. Bonferroni adjustment across the four prespecified aggregate endpoints leaves the M-F1 result below 0.005 ($P=0.0011$); the other endpoints remain non-confirmatory. These tests are not generalized to every exploratory per-class comparison.
...
For the audit rows, binary branch conflict is the indicator $\mathbb{1}[|\{\hat y_r,\hat y_p,\hat y_f\}|>1]$ that at least two of the RNA, ADT and fusion branch top labels differ. The separate three-seed stability diagnostic uses continuous vote dispersion $1-\mathrm{modal\ count}/3$. Final uncertainty is one minus the final maximum softmax probability. Top-probability variance is the variance of the three branch top-class probabilities, and margin variance is the variance of the three branch top-two logit margins. Risk at 90\% coverage sorts cells by the corresponding audit score, accepts the 90\% lowest-risk cells within each donor--seed unit, and reports the error rate among accepted cells. Ties are resolved by the stable input order and do not affect donor-level means at the reported precision.

\begin{table}[H]
\centering
\caption{Expanded error-audit scores. Results are donor-level means with 95\% confidence intervals after averaging seeds within each donor. Risk-at-50\% rows are pooled operating points from the risk--coverage artifact and are shown without a donor-level interval. Binary top-label conflict is a discrete function of the three branch top labels; top-probability variance and margin variance are continuous branch-derived scores available from the same cell-level audit artifact. The canonical direct rows are read from the v46 cell-level audit artifact without fitting. The displayed two- and three-feature rows are predeclared target-donor-held-out logistic feature subsets; they are diagnostics and are not the canonical four-feature estimate. For each target donor, the canonical combined logistic score is fitted on out-of-fold predictions from the seven non-held-out donor folds using final uncertainty, branch conflict, top-probability variance and margin variance, then evaluated on the target held-out donor. The 0.0563 and 0.0565 values refer to final uncertainty and the combined score at 90\% coverage, respectively; the 50\% operating points are shown separately.}
...
\caption{Risk--coverage curves for error auditing. Each curve accepts the lowest-risk cells according to the score on the x-axis coverage fraction and reports the held-out donor error risk among accepted cells. Bands show 95\% intervals across eight donors after averaging seeds. The combined LODO score improves the low-coverage region and remains similar to final uncertainty near 90\% coverage. Because binary branch conflict has only two score values, its curve can remain flat after the non-conflict cells are exhausted; the continuous vote-dispersion diagnostic is reported separately.}
...
Twenty-four full checkpoints were evaluated under eight deterministic conditions without retraining or test-label policy selection. Random masks remove the same fixed feature subset for all cells within a fold/seed condition. Marker masks remove six memory-associated or 15 T-cell-associated ADTs after alias normalization. V8.2 added random masks with exactly six or 15 eligible ADTs per donor--seed unit; these size-matched masks were generated without test labels, predictions, thresholds or test errors.

\begin{table}[H]
\centering
\caption{Frozen-checkpoint performance and calibration under protein-panel masks. Intervals use held-out donor after averaging seeds for the original panel matrix. ECE is top-label expected calibration error from 15 equal-width confidence bins; Brier is the per-cell mean of the multiclass sum of squared probability errors. For ECE and Brier, the worst donor is the maximum value. Marker rows remove six memory-associated or 15 T-cell-associated ADTs; the table explicitly reports removed counts and fractions so targeted and random damage are not conflated. For macro-F1 rows, $\Delta$M-F1 per removed fraction is $(M\!-\!F1_{\mathrm{full}}-M\!-\!F1_{\mathrm{scenario}})/\mathrm{removed\ fraction}$. The V8.2 size-matched rows use 24 frozen donor--seed units, report intervals across those units and leave the worst-donor field unestimated. The final two rows are a separate validation-only scalar-temperature audit on the untouched full-input checkpoints: the temperature was fitted by validation negative log likelihood per donor--seed unit and then applied to test probabilities, without changing the argmax predictions.}
\label{tab:panel}
\scriptsize
\setlength{\tabcolsep}{2.8pt}
\resizebox{\textwidth}{!}{%
...
The donor-level W-F1 degradation slope over 0/10/30/50/70\% random masking was $-0.0263\pm0.0071$ per fully missing protein fraction. The M-F1 slope was $-0.0624\pm0.0108$. Brier score increased by $0.0411\pm0.0097$ per full missing fraction. ECE slope was less stable ($+0.0081\pm0.0072$), whereas the RNA-only ECE increase was clear. Under RNA-only input, the protein vector is zeroed after train-only standardization, so the fused path approaches but is not identical to the RNA branch; its W-F1 differs from the RNA branch by less than $5\times10^{-4}$, while M-F1 is 0.7558 versus 0.7564. Residual differences reflect fusion-layer bias terms and learned RNA--ADT interaction weights. The V8.2 size-matched null shows that targeted six- and 15-feature masks have larger average W-F1/M-F1 losses than same-count random masks, strengthening the targeted-panel interpretation without converting it into arbitrary missing-panel robustness. A separate training-only panel-aware intervention is evaluated below. The complete V8.2 unit-level artifact is \texttt{results/experiments/v8.2\_missing\_modality\_pdc\_audit/pbmc\_random\_adt\_null/}.
...
MMoCHi exceeded MOSAIC for every aggregate metric and most classes. The largest gaps occurred at CD8 central-memory/effector-memory boundaries, while monocyte F1 was identical. Representative ADT evidence is present in both tasks, so marker availability alone does not explain the PDC101 gap; hierarchy and label-space alignment remain protocol-specific alternative explanations. MMoCHi intervals are cell bootstrap intervals conditional on one official workflow fit; MOSAIC intervals are Student-$t$ margins over three model seeds.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Fig/FigS_case_study_marker_limits.pdf}
\caption{Case-study evidence for marker-limited behavior. (a) B naive lambda was the hardest leave-class-out target, with low unknown recall, below-chance AUROC and low parent-safe rate; representative RNA features were present but were not among the selected top-20 attribution hits, and the ADT panel contained no kappa/lambda-like marker. (b) On PDC101, the largest MMoCHi--MOSAIC F1 gaps were concentrated in CD8 memory/effector-memory-related classes, whereas monocyte F1 was identical.}
\label{fig:case-study}
\end{figure}

These two cases show how evidence availability shapes model behavior. B naive lambda illustrates a mismatch between feature availability and retained attribution evidence for unknown detection, whereas the PDC101 CD8 boundary illustrates a setting where an official curated hierarchy may better match the label structure than a learned high-dimensional reference classifier.
...
\begin{center}
\textbf{U3. Inference-only throughput and audit-record extraction overhead (unnumbered diagnostic).}
...
\begin{samepage}
\begin{center}
\textbf{U4. V8.2 audit closure (unnumbered table).}
\textit{The V8.2 closure combines the size-matched PBMC null, the exact PDC101 paired comparison and the GSE164378 5P transfer audit. Source variants and scope boundaries are retained in the table rather than pooled into a single performance ranking.}
\scriptsize
\setlength{\tabcolsep}{3pt}
\input{tables/v82/supplement_v82_audit_closure.tex}
\end{center}
\end{samepage}
...
The training-only panel-aware intervention is reported after the fixed breadth matrix as Supplementary Table S24. Each row averages three seeds within each held-out donor and reports two-sided 95\% Student-$t$ intervals across eight donor means. The panel-aware model uses independent 30\% ADT feature dropout during training only; all evaluation masks are deterministic and selected without test labels.
...
The formal matrices contain 96 primary PBMC retraining units (including the matched-protocol MLP), 24 additional parameter-matched capacity-control units, 96 branch-sensitivity retraining units, 24 full-checkpoint ablation units, 192 panel conditions, 24 panel-aware training units with training-only ADT feature dropout, 15 leave-class-out training units, 90 reject operating conditions, 72 full-training classical/tree-baseline donor runs, two full-training CellTypist-compatible rows evaluated over eight donor folds (16 donor-fold evaluations), one fixed-split scANVI run, one fixed-split totalVI run, one donor-paired MOSAIC-versus-XGBoost audit, 48 CLR sensitivity units (24 fold--seed units per method), 48 full-transcriptome CP10K sensitivity units (24 fold--seed units per method) and 24 validation-only temperature-scaling units. V8.2 adds 48 size-matched random-mask condition rows across 24 frozen donor--seed units, an exact 2,098-cell PDC101 paired audit and a 49,147-cell same-study 5P transfer audit. PDC101 also contains three MOSAIC seeds and one official MMoCHi workflow artifact. The secondary breadth matrix in Table~\ref{tab:cross-dataset-breadth} contains 63 method--dataset--metric cells and is covered by the source-audit CSV cited in its caption; each cell retains source paths and the reported MOSAIC aggregation variant. Versioned script names (\texttt{v30}, \texttt{v33}, \texttt{v36}, \texttt{v50}, \texttt{v7}) denote artifact-generation snapshots rather than manuscript versions; the manuscript reports the current evidence registry and the user-owned Figure~1 asset separately. Each serious training run saves configuration, log, checkpoint, prediction, per-class metrics and artifact index.
...
\item \url{experiments/exp_missing_modality/mosaic_n_v82/run_size_matched_random_adt_null.py} and \url{experiments/exp_missing_modality/mosaic_n_v82/test_size_matched_random_adt_null.py};
\item \url{experiments/exp_generalization/mosaic2_pdc101/run_pdc101_paired_audit.py} and \url{experiments/exp_generalization/mosaic2_pdc101/run_secondary_cohort_audit_transfer.py};
\item \url{results/experiments/v8.2_missing_modality_pdc_audit/experiment_manifest.json}.
\end{itemize}

The release bundle stages code, configurations, ten transformed demonstration cells, compact evidence tables, V8.2 audit summaries, commands, environment metadata and SHA-256 hashes. It excludes raw participant data, full caches, full predictions and checkpoints. The currently verified public GitHub snapshot is tagged \texttt{v8.1.4-submission} under the MIT License; the V8.2 closure artifacts are included in the current submission source and local release candidate, while a public V8.2 tag is not asserted until its remote availability is verified. An immutable archive DOI and raw-data archive are not part of the verified public snapshot. The panel-aware, RNA-CP10K and V8.2 audit artifacts are included as compact evidence records rather than as raw-data redistribution.

\section{AI-assisted tools disclosure}

OpenAI Codex and the ChatGPT interface were used during manuscript preparation for code navigation, experiment-script scaffolding, artifact-audit assistance, table and metadata generation, language editing and figure-prompt iteration. The tools were not treated as sources of scientific evidence and did not determine the dataset split, held-out labels, model-selection rule, statistical test or final conclusion. Human authors selected the protocols, inspected source data and figures, executed or reviewed the analyses, verified the generated artifacts and approved all manuscript text, citations and claims. Figure~1 remains the author-owned final asset; any AI-assisted visual ideation was subsequently reviewed and redrawn or selected by the authors. The exact V8.2 commands, configurations, logs and summary artifacts are listed in U4 and the cited experiment directories.
