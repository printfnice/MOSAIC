...
def write_manuscript_fragments(summary: pd.DataFrame, targets: pd.DataFrame) -> None:
    ...
    for label in sorted(summary["class_label"].unique()):
        ...
        rows.append(
            f"{tex_escape(label)} & {group} & {available} & {int(rna['top20_marker_hits'])} & {int(adt['top20_marker_hits'])} & {float(adt['mean_f1']):.4f} & {float(adt['mean_fusion_weight']):.3f} \\\\"
        )
    target_rows = []
    ...
    for pbmc_label, pdc_label in mapping.items():
        ...
        delta = float(mmochi["mean_f1"] - mosaic["mean_f1"])
        cd8_summary.append(
            {
                ...
                "mosaic_f1": float(mosaic["mean_f1"]),
                "mmochi_f1": float(mmochi["mean_f1"]),
        ...
        cd8_rows.append(
            f"{tex_escape(pbmc_label)} & {tex_escape(pdc_label)} & {int(adt['n_available_panel_markers'])}/{int(adt['n_representative_markers'])} & {int(adt['top20_marker_hits'])} & {float(adt['mean_fusion_weight']):.3f} & {float(mosaic['mean_f1']):.4f} & {float(mmochi['mean_f1']):.4f} & {delta:.4f} \\\\"
        )
    ...
    content = r"""% Generated by build_v8_marker_mechanism_audit.py; no S-number consumed.
